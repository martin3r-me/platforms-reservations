<div class="min-h-screen bg-gray-50 dark:bg-gray-950">
    ...
    <div class="mx-auto max-w-3xl px-4 py-6">
                    ...
                    @foreach (['Gastdaten', 'Produkte', 'Sitzplatz', 'Bezahlung'] as $i => $label)
                        ...
                        <div class="relative flex flex-col items-center">
                            ...
                            <div class="relative z-10 flex h-8 w-8 items-center justify-center rounded-full text-sm font-bold ring-4 ring-gray-50 dark:ring-gray-950
                                {{ $isDone || $isActive ? 'bg-[var(--ui-primary)] text-white' : 'bg-white text-gray-400 border border-gray-300 dark:border-gray-700 dark:bg-gray-900' }}">
                                @if ($isDone)
        ...
        {{-- ── Schritt 2: Produkte je Pause ────────────────────── --}}
        @if ($step === 2)
            <div class="space-y-4">
                <h2 class="text-lg font-semibold dark:text-white">Was darf es in der Pause sein?</h2>

                {{-- Pausen-Auswahl (nur bei mehreren Pausen) --}}
                @if ($this->event->slots->count() > 1)
                    <div>
                        <p class="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Pause wählen – je Pause eigene Auswahl</p>
                    ...
                    <div wire:key="cat-{{ $categoryName }}">
                        ...
                        <div class="space-y-2">
                            @foreach ($items as $item)
                                @php $qty = $carts[$currentSlotId][$item->id] ?? 0; @endphp
                                <div wire:key="prod-{{ $currentSlotId }}-{{ $item->id }}"
                                    ...
                                    <div class="flex shrink-0 items-center gap-2">
                                        @if ($qty > 0)
                                            ...
                                            <span class="w-6 text-center text-sm font-medium tabular-nums dark:text-white">{{ $qty }}</span>
                                        @endif
        ...
        {{-- ── Schritt 3: Sitzplatz je Pause ───────────────────── --}}
        @if ($step === 3)
            <div class="space-y-4">
                <h2 class="text-lg font-semibold dark:text-white">Wo möchten Sie sitzen?</h2>

                {{-- Pausen-Auswahl (nur bestellte Pausen; je Pause ein Tisch) --}}
                @php $orderedIds = $this->slotCarts->keys(); @endphp
                @if ($orderedIds->count() > 1)
                    <div>
                        <p class="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Pro Pause einen Tisch wählen</p>
                        <div class="flex flex-wrap gap-2">
                            @foreach ($this->event->slots as $slot)
                                @continue (!$orderedIds->contains($slot->id))
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach ($this->openRooms as $room)
                                <button wire:key="room-{{ $currentSlotId }}-{{ $room->id }}" wire:click="selectRoom({{ $room->id }})"
                                    class="rounded-xl border px-4 py-2 text-sm {{ ($slotRooms[$currentSlotId] ?? null) === $room->id ? 'border-[var(--ui-primary)] bg-[var(--ui-primary-10)] font-semibold text-[var(--ui-primary)]' : 'border-gray-300 text-gray-700 dark:border-gray-700 dark:text-gray-300' }}">
                                    {{ $room->floorPlan->name }}
                                </button>
                            @endforeach
                        </div>
                ...
                <div class="flex gap-3">
                    ...
                    <button wire:click="nextStep"
                        class="flex-1 rounded-xl bg-[var(--ui-primary)] py-3 text-base font-bold text-white hover:opacity-90">
        ...
        @if ($step === 4)
            <div class="mx-auto max-w-lg space-y-4">
                ...
                {{-- Warenkorb je Pause --}}
                @foreach ($this->slotCarts as $slotId => $lines)
                    @php
                        $slot  = $this->event->slots->firstWhere('id', $slotId);
                        $table = $this->chosenTables->get($slotTables[$slotId] ?? 0);
                    @endphp
                ...
                {{-- Summen --}}
                <div class="rounded-xl border bg-white px-4 py-3 dark:border-gray-700 dark:bg-gray-900">
                ...
                {{-- Altersbestätigung (16+/18+, je nach Warenkorb) --}}
                @if ($this->requiresAgeCheck)
                    <label class="flex cursor-pointer items-start gap-3 rounded-xl border border-purple-300 bg-purple-50 p-3 text-sm dark:border-purple-800 dark:bg-purple-900/20">
                        ...
                        <span class="dark:text-white">
                            <strong>Ich bestätige, dass ich mindestens {{ $this->requiredMinAge }} Jahre alt bin.</strong>
                            <span class="mt-1 block text-xs text-gray-500 dark:text-gray-400">{{ $this->checkoutTexts->ageText() }}</span>
                        </span>
                    </label>
        ...
        @if ($step === 5)
            <div class="mx-auto max-w-lg space-y-4 py-8 text-center">
                ...
                @if ($orderUuid)
                    <p class="text-xs text-gray-500">Bestellnummer: <code class="rounded bg-gray-100 px-1.5 py-0.5 dark:bg-gray-800">{{ $orderUuid }}</code></p>
                @endif
