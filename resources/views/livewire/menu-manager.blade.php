            ...
            @if ($itemIsBundle)
                ...
                <div class="rounded-[8px] border border-[color:var(--nx-line)] p-3">
                    ...
                    <div class="mt-3 space-y-1">
                        ...
                        @empty
                            <p class="m-0 text-[11px] text-[color:var(--nx-muted)]">Noch kein Artikel gewählt.</p>
                        @endforelse
                        ...
                        <div class="mt-1 max-h-52 overflow-auto rounded-[6px] border border-[color:var(--nx-line)]">
                            @forelse ($this->componentCandidates->take(\Platform\Reservation\Livewire\MenuManager::COMPONENT_LIMIT) as $cand)
                                <button type="button" wire:click="addComponent({{ $cand->id }})" wire:key="cand-{{ $cand->id }}"
                                    ...
                                    @else
                                        <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-[6px] bg-[color:var(--nx-bg)] text-[color:var(--nx-faint)]">
                                            @svg('heroicon-o-photo', 'w-4 h-4 opacity-40')
                                        </div>
                                    @endif
                                    <span class="min-w-0 flex-1">
                                        <span class="block truncate text-sm text-[color:var(--nx-text)]">{{ $cand->name }}</span>
                                        @if ($cand->portion_size)
                                            <span class="block text-[11px] text-[color:var(--nx-muted)]">{{ $cand->portion_size }}</span>
                                        @endif
                                    </span>
                                    <span class="whitespace-nowrap text-[11px] tabular-nums text-[color:var(--nx-muted)]">
                                        {{ number_format($cand->price, 2, ',', '.') }} € · {{ rtrim(rtrim(number_format($cand->tax_rate, 2, ',', '.'), '0'), ',') }} %
                                    </span>
                                </button>
                            @empty
